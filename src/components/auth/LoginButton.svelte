<script lang="ts">
  ...
  let providers = $state<SocialProviders>({ github: false, google: false, facebook: false })

  const t = $derived(translations[lang])
  const hasSocialProviders = $derived(providers.github || providers.google || providers.facebook)
    ...
    {#if showMenu}
      {#if mode === 'menu'}
        <div class="menu" role="menu" onclick={(e) => e.stopPropagation()}>
          ...
          <button type="button" class="menu-item" onclick={handlePasskeyLogin} role="menuitem">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
          ...
          {#if hasSocialProviders}
            ...
            {#if providers.github}
              <button type="button" class="menu-item" onclick={loginWithGitHub} role="menuitem">
            ...
            {/if}
            {#if providers.google}
              <button type="button" class="menu-item" onclick={loginWithGoogle} role="menuitem">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            {#if providers.facebook}
              <button type="button" class="menu-item" onclick={loginWithFacebook} role="menuitem">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      ...
      {:else if mode === 'login'}
        <div class="menu" onclick={(e) => e.stopPropagation()}>
          <form onsubmit={handleEmailLogin} class="form">
            ...
            <label class="label">
              {t.email}
              <input type="email" bind:value={email} class="input" required autocomplete="email" />
            </label>

            <label class="label">
              {t.password}
              <input type="password" bind:value={password} class="input" required autocomplete="current-password" minlength="8" />
            </label>
            ...
            <p class="switch-mode">
              {t.noAccount}
              <button type="button" onclick={() => mode = 'signup'}>{t.signUp}</button>
            </p>
          </form>
        </div>
      {:else if mode === 'signup'}
        <div class="menu" onclick={(e) => e.stopPropagation()}>
          <form onsubmit={handleEmailSignUp} class="form">
            ...
            <label class="label">
              {t.name}
              <input type="text" bind:value={name} class="input" required autocomplete="name" />
            </label>
            ...
            <label class="label">
              {t.password}
              <input type="password" bind:value={password} class="input" required autocomplete="new-password" minlength="8" />
            </label>
            ...
            <p class="switch-mode">
              {t.haveAccount}
              <button type="button" onclick={() => mode = 'login'}>{t.signIn}</button>
            </p>
          </form>
        </div>
      {/if}
